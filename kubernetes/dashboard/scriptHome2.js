...
async function fetchAndDisplayDevices() {
    try {
        ...
        for (const circuitId of circuitIds) {
            ...
            const response = await fetch(`http://middleware-service.default.svc.cluster.local:80/circuits/${circuitId}/devices`);
...
async function updateDevice(deviceId, circuitId) {
    try {
        const response = await fetch(`http://middleware-service.default.svc.cluster.local:80/circuits/${circuitId}/sensor/${deviceId}/last`);
...
async function toggleDeviceStatus(circuitId, actuatorId) {
    try {
        ...
        const response = await fetch(`http://middleware-service.default.svc.cluster.local:80/circuits/${circuitId}/actuator/${actuatorId}/toggle`, { 
            method: 'POST' 
        });
        const updatedData = await response.json();
